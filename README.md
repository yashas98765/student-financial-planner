...
This repository includes a Render Blueprint config at `render.yaml` for backend deployment.

1. Push the latest code to GitHub.
2. In Render, click **New +** -> **Blueprint** and select this repository.
3. Render will detect `render.yaml` and create `student-financial-planner-backend`.
4. In the service environment settings, set required values:
   - `MONGODB_URI`
   - `FRONTEND_URL`
   - Optionally override `JWT_SECRET` (auto-generated by blueprint)
5. Deploy and verify health endpoint:
   - `https://<your-render-service>.onrender.com/api/health`

Reference env template: `backend/.env.render.example`
